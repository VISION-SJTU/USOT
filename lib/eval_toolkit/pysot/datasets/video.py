...
class Video(object):
    ...
    def load_img(self):
        if self.imgs is None:
            self.imgs = [cv2.imread(x) for x in self.img_names]
            if self.depth_img_names is not None:
                self.depth_imgs = [cv2.imread(x, cv2.IMREAD_GRAYSCALE) for x in self.depth_img_names]
            self.width = self.imgs[0].shape[1]
            self.height = self.imgs[0].shape[0]
    ...
    def __getitem__(self, idx, depth_tag=None):
        if depth_tag is None:
            ...
            if self.imgs is None or self.depth_imgs is None:
                return cv2.imread(self.img_names[idx]), \
                       cv2.imread(self.depth_img_names[idx], cv2.IMREAD_GRAYSCALE), self.gt_traj[idx]
            else:
                return self.imgs[idx], self.depth_imgs[idx], self.gt_traj[idx]

    def __iter__(self):
        if self.depth_imgs is None:
            ...
            for i in range(len(self.img_names)):
                if self.imgs is not None and self.depth_imgs is not None:
                    yield self.imgs[i], self.depth_imgs[i], self.gt_traj[i]
                else:
                    yield cv2.imread(self.img_names[i]),
                    cv2.imread(self.depth_img_names[i], cv2.IMREAD_GRAYSCALE), self.gt_traj[i]
