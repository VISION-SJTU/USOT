...
parser.add_argument('--output_dir', type=str,
                    default='/home/jlzheng/dataset/tracking/usot/got10k_flow/', help='your got10k anno file output dir')
...
parser.add_argument('--prohibit', type=str, default=None, help='prohibit list required by VOT2020')
args = parser.parse_args()

print('loading json (raw got10k info), please wait 20 seconds~')
got10k = json.load(open(args.anno_file.format(args.bbox), 'r'))
output_dir = args.output_dir.format(args.bbox)
if args.prohibit is not None:
    fopen = open(args.prohibit, 'r')
    lines = fopen.readlines()
    prohibit_1000 = [line.replace('\n', '') for line in lines]
...
def gen_json():
    ...
    n_snippets = 0
    n_videos = 0
    for s_i in range(len(got10k)):
        for video in got10k[s_i]:
            n_videos += 1
            frames = video['frame']
    ...
    if args.bbox == 'flow':
        ...
        if args.prohibit is not None:
            prohibit_num = 0
            for video_name in prohibit_1000:
                try:
                    del snippets["train/" + video_name]
                    prohibit_num += 1
                except:
                    pass
            print("Prohibit number: {}.".format(prohibit_num))
        ...
        for video_name in should_drop_videos:
            try:
                ...
                pass
        print("Filter out number: {}.".format(del_num))

    train = {k: v for (k, v) in snippets.items() if 'train' in k}
    val = {k: v for (k, v) in snippets.items() if 'val' in k}
    all_train_val = {k: v for (k, v) in snippets.items()}

    json.dump(train, open(join(output_dir, 'train.json'), 'w'), indent=4, sort_keys=True)
    json.dump(val, open(join(output_dir, 'val.json'), 'w'), indent=4, sort_keys=True)
    json.dump(all_train_val, open(join(output_dir, 'all.json'), 'w'), indent=4, sort_keys=True)
    print('done!')
