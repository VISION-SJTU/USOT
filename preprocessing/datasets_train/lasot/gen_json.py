...
parser.add_argument('--output_dir', type=str,
                    default='/home/jlzheng/dataset/tracking/usot/lasot_flow/', help='your lasot anno file output dir')
...
lasot = json.load(open(args.anno_file.format(args.bbox), 'r'))
output_dir = args.output_dir.format(args.bbox)


def should_drop(drop_classes, key):
    for item in drop_classes:
        if item == key:
            return True
    return False
...
def gen_json():
    ...
    n_snippets = 0
    n_videos = 0
    for s_i in range(len(lasot)):
        for video in lasot[s_i]:
            n_videos += 1
            frames = video['frame']
            ...
            if len(id_set) > 0 and video['base_path'] not in snippets:
    ...
    drop_classes = ['coin/coin-19', 'coin/coin-1']
    train = {k: v for (k, v) in snippets.items() if not should_drop(drop_classes, k)}
