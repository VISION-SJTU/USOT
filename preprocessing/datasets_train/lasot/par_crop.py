...
import sys, time, os, cv2, argparse, json
from os.path import isdir, join

parser = argparse.ArgumentParser()
parser.add_argument('--input_dir', type=str, default='/data/dataset/lasot', help='your lasot data dir')
parser.add_argument('--anno_file', type=str, default='lasot_{}.json', help='the parsed lasot annotation file')
parser.add_argument('--output_dir', type=str,
                    default='/home/jlzheng/dataset/tracking/usot/lasot_{}', help='your lasot output dir')
...
parser.add_argument('--num_threads', type=int, default=12, help='your number of threads')
args = parser.parse_args()

lasot_base_path = args.input_dir
lasot_benchmark_path = join(lasot_base_path, "LaSOTBenchmark")
sub_sets = sorted(os.listdir(lasot_benchmark_path))
...
def crop_video(video, crop_path, instanc_size):
    ...
    video_file_base = join(lasot_benchmark_path, video['base_path'])

    for frame in video["frame"]:
        frame_file_path = join(video_file_base, "img", frame["img_path"])

        im = cv2.imread(frame_file_path)
        avg_channels = np.mean(im, axis=(0, 1))

        # Use generated bbox in parse_vid.py
...
# def crop_video_check(video, crop_path, instanc_size):
#     video_crop_base_path = join(crop_path, video['base_path'])
#     if not isdir(video_crop_base_path):
#         os.makedirs(video_crop_base_path)
#
...
#             z, x = crop_like_SiamFC(im, bbox, instanc_size=instanc_size, padding=avg_channels)
#
#             test_path = join(video_crop_base_path, '{:06d}.{:02d}.z.jpg'.format(int(filename), track_id))
#
#             if os.path.isfile(test_path):
#                 done_id.append(track_id)
#                 continue
#
#             cv2.imwrite(join(video_crop_base_path, '{:06d}.{:02d}.z.jpg'.format(int(filename), track_id)), z)
#             cv2.imwrite(join(video_crop_base_path, '{:06d}.{:02d}.x.jpg'.format(int(filename), track_id)), x)
...
def main(instanc_size=511, num_threads=24):
    ...
    assert len(anno) == len(sub_sets)

    if not isdir(crop_path):
        os.makedirs(crop_path)

    for sub_i in range(len(anno)):
        anno_subset = anno[sub_i]
        n_videos = len(anno_subset)
        with futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
            fs = [executor.submit(crop_video, video, crop_path, instanc_size) for video in anno_subset]
            for i, f in enumerate(futures.as_completed(fs)):
                # Write progress to error so that it can be seen
                printProgress(i, n_videos, prefix=sub_sets[sub_i], suffix='Done ', barLength=40)
